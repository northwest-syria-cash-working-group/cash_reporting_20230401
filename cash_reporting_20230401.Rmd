...
As of 11 April 2023, a total of `r cbr %>% filter(project_status == "Completed") %>% {sum(.$beneficiaries)} %>% format(big.mark = ",")` persons or `r cbr %>% filter(project_status == "Completed") %>% {sum(.$families)} %>% format(big.mark = ",")` families have been reached by MPC interventions. 
...
`r filter(cbr, month == 2 & project_status == "Completed") %>% {sum(.$beneficiaries, na.rm = TRUE)} %>% format(big.mark = ",")` beneficiaries were reached in February 2023 and `r `filter(cbr, month == 3 & project_status == "Completed") %>% {sum(.$beneficiaries, na.rm = TRUE)} %>% format(big.mark = ",")` were reached in March 2023. `r `filter(cbr, month == 4 & project_status == "Completed") %>% {sum(.$beneficiaries, na.rm = TRUE)} %>% format(big.mark = ",")` have been reached so far in April 2023.Changes in numbers of persons reached were due to cleaning and verification of the dataset -- this process is still ongoing and will only be completed once the CWG fully transitions to 4W reporting. 
...
The `r format(filter(cbr, response_type == "HRP" & project_status == "Completed") %>% {sum(.$beneficiaries, na.rm = TRUE)}, big.mark = ",")`` beneficiaries reached earlier that 2023-02-09 all pertained to the HRP, the only such beneficiaries reported so far. 
...
The CWG is currently working on a prioritisation score, which can be a useful shorthand (or crutch), that will take into account pre-existing vulnerability (likely from the MSNA) as well as earthquake impacts. 
...
cbr %>% 
  ...
  footnote(as_paragraph("Only partners who provided more than USD 100/family/month"), 
           i = 1, j = 1, part = "header", inline = TRUE)
